...
| `@chichex/waica` | One-command launcher and the only published package: `npx @chichex/waica` serves the pre-built editor and opens your browser, `waica mcp` serves the MCP server (the bare npm name `waica` is blocked by npm's similarity filter; the binary is still `waica`) |
...
See [`packages/mcp/README.md`](./packages/mcp/README.md) for all nine tools, the absolute `project_path` contract, editor-coexistence guidance, and how to run a generated project while the `@waica/*` libraries are unpublished.
