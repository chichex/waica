...
packages/
  ...
  behaviors/         # library: PlatformerMotor + state sets, Patrol, Health, Spawner…
...
- [x] **Publish to npm** (2026-08-05): `@waica` org created on npmjs; `@waica/engine`, `@waica/behaviors` and `@waica/archetype-platformer` hand-published at 0.3.0 to bootstrap their Trusted Publisher config, and the CLI renamed from `@chichex/waica` to `@waica/cli` so everything lives under the org. Releases now go out through a `vX.Y.Z` tag, all four packages in lockstep on one version. `@waica/editor` and `@waica/mcp` stay private — they ship inside the CLI. Still open: `waica` org on GitHub, and registering `waica.dev` (optionally `.io`/`.games`).
...
- [x] **Character FSM v1 — runtime** (2026-07-21): `StateMachine` component in the engine as the single owner of a character's frame (states + transitions as prefab data; triggers `input:`/`timer:`/`signal:`, `'*'` wildcard, chained same-frame transitions; entering a state plays the clip of its name with per-state override). State code lives in named logic sets (`defineStates`, merging = extending), with per-instance `fsm.on()` as escape hatch and did-you-mean errors for unknown sets. `PlatformerMovement`+`PlatformerAnimator` replaced by the passive `PlatformerMotor` (same game feel, now methods states call; coyote/buffer tick in the set's `'*'` always-hook) + the `platformer` set (idle/run/jump/fall sharing one body update that reports signals). Projects register states from `src/states/*.ts` (glob in main.ts). Verified e2e in a real browser: idle→run→dashing (KeyE, `vx=30`, gravity off)→run, jump→fall→land, hazard hurt→respawn — zero console warnings. 182 tests green.
- [x] **Character FSM v1 — editor** (2026-07-21): StateMachine card in the inspector (logic as dropdown of registered sets + custom, initial picker, state list with plain-language ⚠/ⓘ tooltips: missing clip, broken transition, where the code runs) and a state modal (rename with graph-wide retarget, clip override, transitions as three dropdowns — key press from the project's actions / after-seconds / signal). "Create code file" scaffolds a working starter into `src/states/<state>.ts` (platformer template uses the motor); the Explorer Components panel gained a State logic group that opens those files in editable Monaco; the Controls editor gained add/remove custom actions with key capture. The slime joined the system: `Patrol` became passive (`step(dt)`) driven by the new `patroller` set, so every shipped character now runs through a StateMachine. Adding a state auto-opens its modal — the warnings are the user's checklist. Verified e2e in a real browser (demo project): create `dashing` from the card, pick clip/timer via dropdowns, scaffold the code file, add the `dash` action bound to E, wire `key press: dash → dashing` on idle, play-in-editor clean. 195 tests green.
- [x] **Character roles — the behavior remodel** (2026-07-24): a character's behavior is now one answer to "what is this character?" — its **Role**. The FSM's opaque `logic` dropdown (a genre name pointing at invisible code that secretly needed a hand-added Motor) became a role registry in the engine: `defineRole(name, { description, driver, graph, states })`, with `platformer`→**`player`** renamed at every level (UI, prefab JSON `role:`, `defineStates('player')`, scaffolds). Characters are **born whole** — the creation flow asks the role (preselects `player` until the project has one) and installs graph + driver together; switching role swaps the full package behind a confirm that spells out exactly what changes (Cancel = nothing). Each role explains itself: descriptions show in the creation picker, the Role card and the switch confirm. The driver card renders nested under the Role card — one visible package. Custom roles are the level-3 exit: scaffold `src/roles/<name>.ts` from the Role dropdown (self-registers in the game; the editor lists it after TS hot reload lands). `missingDriver` demoted from assembly instructions to a hand-edited-JSON safety net. The registry is N roles : 1 driver by design — next roles it already supports: **walker** (goomba: shares PlatformerMotor with player, AI intentions instead of input), **chaser**, **flyer**.
