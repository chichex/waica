...
Only classes with `onUpdate` participate, and component identity is unique per entity. An invalid graph fails closed for that entity instead of inventing fallback semantics. This replaces ADR-0003's former premise that component update order was prefab-authored. ADR-0003 now uses `Health`'s first eligible update after `StateMachine`; its damage-model decision remains unchanged.
